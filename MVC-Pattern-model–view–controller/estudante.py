...
class LimiteConsultaEstudantes(tk.Toplevel):
    ...
    def framesTela(self, controle):
        ...
        self.labelNro = tk.Label(self.frameNro, text="Matricula")
        self.labelNro.pack(side='left')
        self.inputNro = tk.Entry(self.frameNro, width=20)
        self.inputNro.pack(side='left')
...
class CtrlEstudante():       
    ...
    def consultHandler(self, event):
        ...
        est = None
        try:
            for mat in self.listaEstudantes:
                if mat.getNroMatric() == nroMatric:
                    analise = False
                    est = mat
            ...
            self.limiteCon.mostraJanela('Erro', 'Matrícula inválida!')
        
        else:
            str = 'Nro Matric. -- Nome\n'
            str += est.getNroMatric() + ' -- ' + est.getNome() + '\n'
            self.limiteCon.mostraJanela('Estudante', str)
