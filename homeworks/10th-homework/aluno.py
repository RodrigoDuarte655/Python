...
class LimiteMostraAlunos(tk.Toplevel):
    def __init__(self, str):
        messagebox.showinfo('Alunos cadastrados', str)
...
class CtrlAluno():
    ...
    def printHistoricoDiscEltv(self, aluno):
        ...
        if len(aluno.getDisciplinasEletivas()) == 0:
            ...
            str += ('\nO não possui carga horária cadastrada para disciplinas eletivas')
        else:
    ...
    def insereHandler(self, event):
        try:
            ...
            self.limiteIns.mostraJanela("Cadastro não permitido", "Já existe um aluno com este número de matrícula!")
            self.limiteIns.inputNro.delete(0, len(self.limiteIns.inputNro.get()))
        else:
