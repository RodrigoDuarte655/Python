...
class CtrlHistorico():
    ...
    def consultaHandler(self, event):
        try:
            ...
            self.limiteIns.mostraJanela('Erro', 'Digite um nome ou Nro de Matrícula!')
            ...
            if aluno == None:
                ...
                self.limiteIns.mostraJanela('Aluno não encontrado', str)
    ...
    def insereHandler(self, event):
        try:
            ...
            self.limiteIns.mostraJanela('Algo errado', 'Para que registrar um histórico tão antigo?')
            self.janelaHistorico.inputAno.delete(0, len(self.janelaHistorico.inputAno.get()))
        except NotaInvalida:
            str = 'Nota inválida!\n'
            ...
            self.limiteIns.mostraJanela('Histórico já inserido', 'Esta disciplina já foi cursada por este aluno neste mesmo semestre!')
            ...
            try:
                for disc in listaDiscGradeAluno:
                    if historico.getDisciplina().getCodigo() == disc.getCodigo():
                        aluno.addDisciplinaObrigatoria(historico.getDisciplina())
                        self.limiteIns.mostraJanela('Sucesso', 'Histórico adicionado!')    
                        self.limiteIns.destroy()
                    else:
                ...
                for grad in self.CtrlPrincipal.ctrlGrade.getListaGrades():
                    if grad.getCodigo() != aluno.getCurso().getGrade().getCodigo():
                        for disc in grad.getDisciplina():
                            if disc.getCodigo() == historico.getDisciplina().getCodigo():
                        ...
                        self.limiteIns.mostraJanela('Tem algo errado', str)           
